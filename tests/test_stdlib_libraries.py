"""Guardrail: every Op library shipped under clops/stdlib imports and lints.
...
the stdlib libraries (the CLI lint tests use their own fixtures), and
`clops.stdlib.code_review` shipped broken twice because of it: first importing
its siblings by bare name (`from code_review import ...`), then with no `Meta`
on any of its eleven Ops.

Discovery is dynamic, so a new library added under clops/stdlib is covered the
moment it lands.
"""
...
STDLIB_LIBRARIES = sorted(
    m.name for m in pkgutil.iter_modules(clops.stdlib.__path__) if m.ispkg
)


def test_stdlib_libraries_are_discovered():
    """Guard the guard: empty discovery would make everything below vacuous."""
    assert STDLIB_LIBRARIES, "no packages found under clops/stdlib"
    assert "code_review" in STDLIB_LIBRARIES


@pytest.mark.parametrize("library", STDLIB_LIBRARIES)
def test_stdlib_library_imports(library):
    ...
    importlib.import_module(f"clops.stdlib.{library}")


@pytest.mark.parametrize("library", STDLIB_LIBRARIES)
def test_stdlib_library_registers_ops_and_lints(library):
    ...
    result = check_library(f"clops.stdlib.{library}")
    assert result.ok, "\n".join(str(f) for f in result.errors)
    assert registry.ops(), f"clops.stdlib.{library} registered no Ops"
